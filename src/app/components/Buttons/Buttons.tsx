...
export function AppButton({ content, onClickButton, isRounded, isLoading, type, isDisabled }: AppButtonProps) {
    return (
        <button
            className={`w-full ${isRounded ? 'rounded-full' : 'rounded-[5px]'} py-3 ${isDisabled ? 'bg-[#2b0a6071]' : 'bg-primaryColor'} ${isDisabled ? 'cursor-not-allowed' : 'cursor-pointer'} text-white font-[400] text-[4vw]`}
            type={type}
            disabled={isDisabled}
...
export function OutlinedAppButton({ content, onClickButton, isRounded, isLoading }: AppButtonProps) {
    return (
        <button
            onClick={onClickButton}
            className={`w-full ${isRounded ? 'rounded-full' : 'rounded-[5px]'} py-3 bg-white border border-[#EBECEF] cursor-pointer text-[#17181C] font-[400] text-[4vw]`}>
            {isLoading ? (
                <span className="flex items-center justify-center">
                    <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
